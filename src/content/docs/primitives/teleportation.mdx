...
Se asume que tanto el transmisor como el receptor comparten un cubit entrelazado, es decir que: el transmisor tiene un cubit **A**, el receptor tiene un cubit **B**, y juntos el par **(A,B)** está en el estado $\ket{\Phi^+}$, que es un <a href={`${import.meta.env.BASE_URL}/concepts/entanglement#entrelazamiento-maximal`}>estado de Bell</a>.
...
Dentro de los componentes, se enumeran $5$ pasos o étapas del protocolo, enumerados en un orden que pretende simular la comunicación entre dos agentes.

El primer componente $c_1$ es la **creación de un par entrelazado**, como fue mencionado previamente, conformando el estado $\ket{\Phi^+}$, que es uno de los estados de Bell.
...
- Por último si se lee un $1$ en la primera en la primer linea y un $1$ en la segunda, para que quede igual es necesario invertir las amplitudes y aumentar la fase en $180^\circ$ (aplicar ámbas puertas).
