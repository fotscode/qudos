...
Se asume que tanto el transmisor como el receptor comparten un cubit entrelazado, es decir que: el transmisor tiene un cubit **A**, el receptor tiene un cubit **B**, y juntos el par **(A,B)** está en el estado $\ket{\psi^+}$.
...
- Se entrelaza el cubit que se desea transmitir con la mitad del par entrelazado del transmisor (primer linea con segunda) utilizando la puerta CNOT. Esto causa que el cubit a transmitir se entrelace de forma _indirecta_ con el cubit del receptor (primer linea con tercer linea).
...
El cuarto componente $c_4$ finaliza la teleportación, **recibiendo el mensaje**. El recibir el mensaje implica realizar cambios al cubit recibido según las lecturas, dando cuatro combinaciones posibles:

- En caso de que se lea un $0$ en la primera linea y un $0$ en la segunda linea no es necesario realizar ningún cambio al cubit recibido.
...
<Tabs>
  <TabItem label="Círculos">
        <Box>
            Analizaremos paso a paso el estado de los cubits segun los separadores provistos en el circuito, donde \\(\phi_i\\) es el estado en el paso \\(i\\), empezando con \\(i=1\\) y finalizando con \\(i=10\\).
            ...
            <div style="display: flex; flex-direction:column; align-items:center; margin-bottom:60px">
                Con el paso $\phi_2$ se finaliza la componente $c_1$ creando un par entrelazado ($\psi^+=\frac{1}{\sqrt{2}}(\ket{00}+\ket{11})$) entre el segundo y tercer cubit.
                <div style="display: flex; justify-content:center; align-items:center;flex-direction:row">
                <Circle width={"80"} phase={"0"} prob={0.5} label={"\\ket{000}"}/>
                <Circle width={"80"} phase={"0"} prob={0} label={"\\ket{001}"}/>
                <Circle width={"80"} phase={"0"} prob={0} label={"\\ket{010}"}/>
            ...
            <div style='display: flex; flex-direction:column; align-items:center; margin-bottom:60px'>
              ...
              CNOT, teniendo como control el primer cubit y como objetivo el segundo cubit.
              El resultado de esto es el entrelazamiento del primer cubit con el segundo
              cubit y se crea, de forma indirecta, un entrelazamiento entre el primer cubit
            ...
            <div style='display: flex; flex-direction:column; align-items:center; margin-bottom:60px'>
              Seguido al anterior, se aplica una Hadamard al primer cubit, dejandolo
            ...
            <div style='display: flex; flex-direction:column; align-items:center; margin-bottom:60px'>
              Este paso es crucial ya que realiza el envío (finaliza el componente $c_3$) y define como interpretar el mensaje al receptor. Las lecturas causan una perdida de los primeros dos cubits, estas son necesarias ya que no se pueden copiar los estados cuánticos.
            
            Digamos que el primer cubit, al leerlo, colapsa en un $1$ (convirtiendose en un bit clásico), y supongamos que el segundo cubit al leerlo también colapsa en $1$, obteniendo el siguiente estado cuántico.
            ...
            <div style='display: flex; justify-content:center; align-items:center; flex-direction:row;margin-bottom:30px'>
              ...
              <Aside>
              Es interesante denotar que las lecturas posibles $(00, 01, 10, 11)$ corresponden con las columnas de la notación de círculos, por ejemplo si se hubiese leido un $1$ en el primer cubit y un $0$ en el segundo se hubiese obtenido la tercer columna (finalizando con $10$).
              </Aside>
            </div>
