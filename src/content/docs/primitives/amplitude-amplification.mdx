...
La etapa de **preparación** denota una precondición del algoritmo: que el estado cuántico se encuentre en superposición. La forma en la que será implementada dependerá del problema en cuestión. Generalmente, se habla de un algoritmo cuántico $\mathcal{A}$ que no realiza mediciones durante su ejecución.
...
<Aside>
  Realizar más iteraciones de las necesarias puede resultar contraproducente ya que, si se excede el número óptimo de iteraciones $\left(N_{iteraciones} +1\right)$, el estado cúantico permanecerá seleccionado haciendo que la operación de selección funcione de manera insensata.{' '}
</Aside>
