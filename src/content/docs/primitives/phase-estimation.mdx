...
\end{align}
```
Es util reescribir $\theta=2\pi j$, donde $j=0.j_0j_1\ldots j_{t-1}$ siendo $j$ una fracción en binario, para obtener un formato similar al de $QFT$:
```math
\begin{align}
\ket{\sigma_3}&=2^{-t/2}(\ket{0}+e^{2\pi i 2^{t}(0.j_0j_1\ldots j_{t-1})}\ket{1})(\ket{0}+e^{2\pi i 2^{t-1}(0.j_0j_1\ldots j_{t-1}) }\ket{1})\\&
\ldots(\ket{0}+e^{2\pi i 2^{0}(0.j_0j_1\ldots j_{t-1})}\ket{1})\ket{\psi}
\end{align}
```
Aplicar la operación $2^h(0.j_0j_1\ldots j_{t-1})$ implica realizar un desplazamiento binario (shift) de $h$ posiciones. 
...
```math
\begin{align}
\ket{\sigma_3}&=2^{-t/2}(\ket{0}+e^{2\pi i (0.j_{t-1})}\ket{1})(\ket{0}+e^{2\pi i (0.j_{t-2} j_{t-1}) }\ket{1})\\&
\ldots(\ket{0}+e^{2\pi i (0.j_0j_1\ldots j_{t-1})}\ket{1})\ket{\psi}
\end{align}
```
...
```math
\begin{align}
\ket{\sigma_4}&=QFT^\dag(2^{-t/2}(\ket{0}+e^{2\pi i (0.j_{t-1})}\ket{1})(\ket{0}+e^{2\pi i (0.j_{t-2} j_{t-1}) }\ket{1})\\&
\ldots(\ket{0}+e^{2\pi i (0.j_0j_1\ldots j_{t-1})}\ket{1})\ket{\psi})\\
&=\ket{j}
\end{align}
```
...
```math
\begin{align}
\theta&=2\pi j=2\pi0.j_0j_1\ldots j_{t-1}\\
autovalor &= e^{i \theta}
\end{align}
```
