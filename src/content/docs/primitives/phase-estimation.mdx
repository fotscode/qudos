...
head:
  - tag: meta
    attrs:
      { property: 'og:image', content: /phase-estimation.png }
  - tag: meta
    attrs:
...
La **estimatición de fase (EF)**, es una primitiva cuántica que es utilizada en distintos algoritmoss, permite extraer información legible de superposiciones. Por ejemplo es utilizada en el algoritmo de Shor, el algoritmo de HHL, entre otros. Internamente esta primitiva utiliza la <a href={`${import.meta.env.BASE_URL}/primitives/fourier-transform`}>transformada de Fourier</a> inversa.
