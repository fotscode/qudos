...
Es por esto que en estos casos una buena opción consiste en aplicar uncomputation, una técnica que permite dejar los qubits auxiliares 
...
El proceso de uncomputation involucra tres pasos:
...
$w(x)$ el estado "basura" resultante tras haber ejecutado $s(x)$. Se utiliza el enfoque estándar que consiste en convertir compuertas clásicas AND y NOT en compuertas cuánticas 
Toffoli y NOT, respectivamente
- Agregar un qubit extra en el estado $\ket{0}$, y aplicar la compuerta CNOT sobre $\ket{s(x)}$ como el control. Esto permitirá copiar el resultado y se obtendrá:
$\ket{x}\ket{s(x)}\ket{w(x)}\ket{s(x)}$
- Ahora aplicar todas las compuertas del primer paso pero en orden inverso, y aplicando en cada paso la compuerta inversa. El resultado deshará lo ocurrido en el paso 1, 
...
convirtiendo compuertas clásicas en cuánticas; copiar la respuesta usando CNOT; y finalmente aplicar uncomputing, revirtiendo las compuertas e invirtiéndolas.

Suponiendo que se intenta computar $s(x) = x_1 \land x_2 \land x_3$, es decir, la operación AND de tres bits, se usará una compuerta Toffoli para computar el AND de los 
primeros dos bits, $x_1 \land x_2$:

<div style='display:flex; justify-content:center; align-items:center; flex-direction:column'>
  <Image
    src={`${import.meta.env.BASE_URL}/uncomputation/01.png`}
    alt='Operación AND sobre x1 y x2 usando compuertas Toffoli'
    width='300'
    height='150'
  />
</div>

Luego se usaría otra compuerta Toffoli para aplicar la operación AND entre el resultado de la operación previa y $x_3$:

<div style='display:flex; justify-content:center; align-items:center; flex-direction:column'>
  <Image
    src={`${import.meta.env.BASE_URL}/uncomputation/02.png`}
    alt='Operación AND entre el resultado previo y x3 usando compuertas Toffoli'
    width='300'
    height='200'
  />
</div>
...
Esto puede evitarse fácilmente aplicando uncomputation. Siguiendo los pasos mencionados previamente, y recordando que la compuerta Toffoli es inversa de sí misma:

<div style='display:flex; justify-content:center; align-items:center; flex-direction:column'>
  <Image
    src={`${import.meta.env.BASE_URL}/uncomputation/03.png`}
    alt='Aplicación de uncomputation sobre la operación x1 AND x2 AND x3 usando compuertas Toffoli'
    width='400'
    height='400'
  />
</div>

El costo de aplicar uncomputation es lineal en relación a los números de qubits involucrados en el circuito, ya que deben aplicarse las operaciones inversas a las utilizadas 
...
es posible aplicar uncomputation sobre los circuitos de forma eficiente, utilizando dicha práctica cuando sea requerida con el objetivo de mantener estados de cómputo limpios.
