...
La **transformada de Fourier cuántica (QFT)**, es otra primitiva cuántica, que se asemeja al algoritmo clásico (transformada de Fourier discreta o DFT) la cual es utilizada en distintos algoritmo. Por ejemplo el algoritmo de Shor y la primitiva de <a href={`${import.meta.env.BASE_URL}/concepts/phase-estimation`}>estimación de fases</a> que este mismo utiliza.
...
Un circuito de QFT es posible definirlo utilizando compuertas <a href={`${import.meta.env.BASE_URL}/gates/hadamard`}>Hadamard</a> y compuertas de fase controladas $R_i$.


La compuerta $R_i$ se define de la siguiente manera:
```math
R_i=\begin{bmatrix}
1&0\\
0&e^{2\pi i/2^i}\\
...
Con esto en condiseración el algoritmo $QFT_n$ sigue una estructura facilmente generalizable, donde $n$ es la cantidad de qubits presentes.
...
En este caso se utiliza $j_1, j_2, \ldots, j_n$ para representar los qubits de entrada que conforman el estado cuántico $j$:
```math
j=j_1j_2\ldots j_n
```
...
Se encuentran $n$ compuertas Hadamard y $(n)(n+1)/2$ compuertas de rotación, por lo que el órden del algoritmo es $O(n^2)=O((log_2 N)^2)$, ya que $N=2^n$.

<Aside>La implementación de QFT varía según la bibliografía, se pueden presentar con diferentes compuertas, o en diferente órden, o con/sin compuertas de intercambio. A su vez, existen casos donde se interpreta QFT con la misma definición que DFT y en otros casos como su inversa (ángulos opuestos).</Aside>

Por otra parte, como el algoritmo de QFT es conformado por compuertas unitarias, para implementar la **inversa de QFT** basta con aplicar las operaciones conjugadas transpuestas en el orden inverso.
