...
En la computación cuántica, clonar qubits es más complicado. Supóngase que se dispone de un qubit en un estado 
...
Notar que si se tiene un qubit en un estado cuántico conocido, como por ejemplo $\ket{+}$, pueden producirse copias 
adicionales recurriendo a las compuertas que producen dicho estado: 
...
un qubit en un estado desconocido $\ket{\psi} = \alpha\ket{0} + \beta\ket{1}$, a partir del cual quiere hacerse una copia:

```math
\ket{\psi}\ket{0} \longrightarrow \ket{\psi}\ket{\psi}
```

Asumiendo que existe una operación unitaria $U$ que es capaz de clonar los qubits en dos estados posibles 
...
Say there is a unitary U that is able to clone qubits in two known states |ψ⟩ and
|φ ⟩. That is,
