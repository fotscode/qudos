...
Para lograrlo, utiliza un enfoque híbrido que combina cálculos clásicos con un subrutina cuántica basada en la <a href={`${import.meta.env.BASE_URL}/primitives/phase-estimation`}>estimación de fases</a>.
...
Otra forma de expresar este problema es a partir de la búsqueda del periodo u orden de un número $x^r$ tal que $x^r\ mod\ N=1$, $r$ debe ser lo más chico posible y mayor a $0$. Por ejemplo:
```math
x=4,\ N=5\\
...
4. <Badge text="Cuántico" variant="tip" />: Si $s=1$ *(son coprimos)*, obtener $\widetilde{s/r}$ usando la subrutina cuántica de <a href={`${import.meta.env.BASE_URL}/primitives/phase-estimation`}>estimación de fases</a>.
...
El primer parámetro es facil de preparar, un registro de cubits con $\ket{0}$. El segundo depende la definición de $U$, la cual definiremos a continuación.
...
\end{align}
```
Ahora aplicamos $U$ sobre $\ket{u_s}$ para verificar su autovalor.
```math
\begin{align}
...
Para suplir con el tercer parametro, debemos obtener una instancia de $\ket{u_s}$ el cual ya sabemos que es un autovector de $U$. Para esto construimos una superposición de $\ket{u_s}$ que suple esta necesidad:
```math
\begin{align}
...
Aplicamos el algoritmo de estimación de fases sobre $\ket{0}\ket{1}$. Como el autovalor de $\ket{1}$ es $e^{2\pi i s/r}$ el resultado es:
...
Obtenemos finalmente el valor de $\widetilde{s/r}$ luego de aplicar una lectura en $q_{salida}$. Este valor es estimado al nivel de precisión dependiente de la cantidad de cubits provistos.
...
El primero es el **físico**, al no existir máquinas cuánticas con cubits lógicos suficientes, se **emula** el funcionamiento de estas en computadoras clásicas. Este procedimiento es muy costoso, ya que el crecimiento en operaciones es **exponencial**.
