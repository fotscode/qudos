...
head:
  - tag: meta
    attrs:
      { property: 'og:image', content: /quantum-katas/shor-pe-steps.png }
  - tag: meta
    attrs:
...
- Por otra parte, el **segundo** parámetro es la compuerta controlada de $U$.
...
\end{align}
```
Por otra parte, al saber que $\ket{u_s}$ es un autovector de $U$, ocurre el fenómeno de **retroceso de fase** cuando se aplica la compuerta controlada de $U$:
```math
...
El segundo problema es la implementación de la **exponencialización modular**, en el ejemplo previo se utilizó una compuerta que calculaba el módulo con $N=15$ y se repitió de forma exponencial la utilización de esta compuerta según el qubit, es decir, una vez en el primer qubit, 2 veces en el segundo qubit, 4 veces en el tercero, etc.

La aplicación exponencial de las compuertas controladas de módulo **deshacen** la ventaja exponencial provista por el algoritmo. Existen **circuitos eficientes**$^{[1][2]}$ que resuelven este problema, pero dado su complejidad y su lentitud al emularlo, se decidió evitar la implementación de un algoritmo general de Shor usando *Qiskit*.
