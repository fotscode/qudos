...
La clave del algoritmo radica en la identificación del período de la función $f(r) = x^r\ mod\ N$, donde $N$ es el número que queremos factorizar, $x$ es un numero elegido al azar que es coprimo de $N$ (es decir, $x$ y $N$ no comparten factores primos), y $1<x<N$.
...
\vdots
```
En este caso $r=2$, por lo que cada 2 valores se repite la secuencia. Se obtienen los factores de $N$ de la siguiente manera:
```math
