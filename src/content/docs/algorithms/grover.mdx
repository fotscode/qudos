...
El algoritmo de Grover es el algoritmo de búsqueda cuántico por excelencia, ya que permite encontrar elementos dentro de un conjunto de elementos no estructurado. 
Es muy importante porque puede aplicarse sobre diferentes clases de problemas y acelerar los algoritmos clásicos que usan heurísticas de búsqueda. 
...
Los pasos llevados a cabo en este algoritmo se corresponden con la metodología general de la <a href={`${import.meta.env.BASE_URL}/primitives/amplitude-amplification`}>amplificación de amplitudes</a>, implementada de manera específica para el problema planteado en este artículo.
...
2. Etapa de **reflejo**. <a href={`${import.meta.env.BASE_URL}/primitives/amplitude-amplification#reflejo`}>Ver detalles de implementación</a>. 

Dado que el efecto de la última etapa se percibe como $2\ket{u}\bra{u} - I$, la iteración de Grover $G$ puede escribirse de la siguiente manera:
