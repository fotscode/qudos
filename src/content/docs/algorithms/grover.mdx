...
no posee estructura alguna. Es muy importante porque puede aplicarse para diferentes tipos de problema y acelerar los algoritmos clásicos 
...
un punto de vista geométrico para visualizar su acción y ver cómo trabaja.
...
to recognize solutions to the search problem. This recognition is signalled by making use of an oracle qubit. More precisely, the oracle is a unitary operator, O, 
...
Además, se supondrá que se dispone de un oráculo cuántico con la habilidad para reconocer soluciones al problema de búsqueda. Esto reconocimiento será reflejado 
mediante un qubit destinado al oráculo. Precisamente, el oráculo es un operador unitario $O$, definido por su acción en la base computacional:

```math
\ket{x}\ket{q} \rightarrow \ket{x}\ket{q\oplus f(x)}
```

donde $\ket{x}$ es el registro índice, $\oplus$ denota suma módulo 2, y el qubit del oráculo $\ket{q}$ es un solo qubit que cambia su valor únicamente si $f(x) = 1$.
Puede verificarse si $x$ es una solución al problema preparando $\ket{x}\ket{0}$ para luego aplicar el oráculo y verificar si el qubit del mismo cambió su valor a $\ket{1}$.

Particularmente, en el algoritmo de búsqueda cuántico es útil aplicar el oráculo con su qubit inicializado en el estado $\frac{\ket{0} - \ket{1}}{\sqrt 2}$ 
(abreviado como $\ket{-}$ por cuestiones de practicidad).
Si $x$ no es una solución al problema, aplicar el oráculo $\ket{x}\ket{-}$ no cambiará su estado. Por otro lado, si $x$ es una solución, entonces $\ket{0}$ y 
$\ket{1}$ serán intercambiados por acción del oráculo, dando el estado resultante $-\ket{x}\ket{-}$. En resumen, la acción del oráculo es:

```math
\ket{x}\ket{-} \rightarrow -1^{f(x)}\ket{x}\ket{-}
```

Notar que el estado del qubit del oráculo no cambia, sino que se mantiene en $\ket{-}$ durante toda la ejecución del algoritmo, y por lo tanto puede ser 
omitido más adelante para tener una mayor simplicidad. Con esta convención, la acción del oráculo puede escribirse como:

```math
\ket{x} \rightarrow -1^{f(x)}\ket{x}
```

Dado el funcionamiento del oráculo, puede decirse que el mismo marca las soluciones del problema de búsqueda, modificando la fase de las soluciones.
...
El algoritmo comienza en el estado $\ket{0}^{\otimes n}$. A continuación, la transformación de Hadamard para alcanzar un estado de superposición igualitaria
...
cuántica, conocida como iteración de Grover, representada mediante la letra $G$, que consta de los siguientes pasos:

1. Aplicar el oráculo O 
2. Aplicar la transformación de Hadamard $H^{\otimes n}$
3. Realizar un cambio de fase condicional, con todos los estados excepto $\ket{0}$ recibiendo un cambio de fase de $-1$,
...
Dado que aplicar $H^{\otimes n}$ da como resultado una superposición uniforme $u$.
...
Se definirán a continuación dos conjuntos de cadenas.
...
Recordar que también se cumple que $\ket{u} = H^{\otimes n}\ket{0^n}$, de manera que $\ket{u}$ representa el estado del registro $Q$ tras realizar el paso 
1 de la iteración de Grover. Esto implica que justo antes de alcanzar el paso 2 de $G$, el estado de $Q$ se encuentra contenido en un espacio vectorial 
de dos dimensiones abarcado por $\ket{A_0}$ y $\ket{A_1}$, y los coeficientes de ambos vectores son números reales. Cabe destacar que el estado de $Q$ siempre
tendrá estas propiedades luego de cualquier cantidad de iteraciones de $G$ en su segundo paso.
...
para $\theta = sin^{-1}(\sqrt{\frac{|A_1|}{N}})$. Utilizando esta nueva expresión para $M$:
...
Retomando, el estado del registro $Q$ al comienzo del paso 2 es 
...
La idea es que la iteración de Grover rote el vector de estado hacia $\ket{A_1}$. Cuando esto ocurre, una medición del estado produce con alta probabilidad 
una de las salidas superpuestas en $\ket{A_1}$, es decir, una solución al problema.

Inicialmente el vector de estado se encuentra en un estado ortogonal respecto de $\ket{A_1}$. Partiendo de ese punto, la operación del oráculo $O$ refleja 
el estado a partir de $\ket{A_0}$, y luego la operación $2\ket{u}\bra{u} - I$ lo refleja en torno a $\ket{u}$. Dado que dos reflexiones producen una rotación,
...
En este apartado se desarrollará sobre la cantidad de iteraciones necesarias para encontrar una solución utilizando el algoritmo de Grover. 
...
Dado que el estado $Q$ luego de $t$ iteraciones en el paso 2 es 
...
tanto como sea posible. Dado que se busca que $sin((2t+1)\theta)$ sea cercano a 1 en valor absoluto, una forma de lograrlo consiste en elegir $t$ tal que 
...
**Insertar imagen de función seno**
...
de encontrar una solución, en base a la cantidad de iteraciones elegida. En este contexto, se buscará alcanzar el valor positivo $t$ cercano a $|1|$ más pequeño, 
con el objetivo de realizar la menor cantidad de iteraciones posible con una gran probabilidad de éxito.
...
En base al resultado obtenido en $t$, puede verse que el algoritmo de Grover requiere a lo sumo $O(\sqrt{N})$ consultas para encontrar la solución.
...
Y puede probarse que en general se cumple 
...
Lo cual reduce la cantidad de consultas requeridas a la función $f$ a medida que $s$ aumenta. El número de consultas requeridas es de $O(\sqrt{\frac{N}{s}})$.
...
Por otro lado, existe un método que encuentra una solución cuando al menos una existe realizando $O(\sqrt{\frac{N}{s}})$ consultas, incluso cuando el número de 
soluciones de $s$ no es conocido. Cabe destacar que se requieren $O(\sqrt{N})$ consultas para determinar que no hay soluciones cuando $s=0$. La idea consiste en 
...
no desperdiciar consultas cuando ya se sabe que no existe una solución. Este procedimiento tiene la ventaja de que require menos consultas cuando hay más soluciones.
...
Hasta este punto se ha asumido que el número de soluciones es distinto de cero. Además, se ha dejado en claro que tanto $A_0$ como $A_0$ eran conjuntos no vacíos. 
En este apartado se explicará qué ocurre cuando alguno de ambos conjuntos se encuentra vacío. 

Si todas las cadenas $x \in \Sigma^n$ son una solución, entonces cualquier medición reflejará una solución; y cuando no haya soluciones, ninguna medición obtendra 
...
Sea $s=11$ la solución única al problema se modifica el oráculo con una función que toma valor 1 únicamente con dicha cadena. Utilizaremos una compuerta <a href={`${import.meta.env.BASE_URL}/gates/cz`}>CZ</a> para que el oráculo detecte la solución única.
