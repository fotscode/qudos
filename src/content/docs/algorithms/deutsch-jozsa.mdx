...
El **algoritmo clásico**, implica recorrer al menos mitad de las salidas posibles. El **algoritmo cuántico** realiza esta operación en **una** sola pasada.

## Versión Reducida

Se plantea en un principio una versión reducida del problema, donde la función que se evalua posee una única entrada:

```math
f: \{0,1\} \rightarrow \{0,1\}
```

Esto permite simplificar el algoritmo en su unidad más básica, favoreciendo a que se comprenda el problema que resuelve el algoritmo. Posteriormente se explicará una versión completa del algoritmo para una función con $n$ entradas.
...
<div style='display:flex; justify-content:center; align-items:center; flex-direction:column'>
  ...
  <span style='width:504px; padding:10px 40px'>
    **Figura (1):** Algoritmo Deutsch-Jozsa en un circuito con separadores ante
...
Por lo que si introducimos esta función/circuito dentro del algoritmo de Deutsch-Jozsa la salida debe ser $\ket{0}$, gráficamente se observaría de la siguiente manera:
...
Otros ejemplos de funciones constantes pueden ser aquellos que niegan ante cualquier input, aplicando una compuerta <a href={`${import.meta.env.BASE_URL}/gates/pauli-x`}>Pauli-X</a>, una vez a su salida. Otra función posible es aquellos que aplican la compuerta dos veces a la salida, otra tres veces, etcétera.
...
Si introducimos esta función/circuito dentro del algoritmo de Deutsch-Jozsa la salida, a diferencia del previo, debe ser $\ket{1}$, gráficamente se observaria de la siguiente manera:
...
Esto determina que, para este ejemplo de función balanceada, se deriva al resultado correcto, es decir que el circuito con $CNOT$ deriva en el resultado $\ket{1}$. También se puede observar en el simulador como cambió el estado de _"Off"_ a _"On"_.
...
Otros ejemplos de funciones balanceadas pueden ser aquellos circuitos que aplican dos veces una compuerta <a href={`${import.meta.env.BASE_URL}/gates/cnot`}>CNOT</a>. Otra función posible es aquellos que aplican la compuerta tres veces a la salida, otra cuatro veces, etcétera.
