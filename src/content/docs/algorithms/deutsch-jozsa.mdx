...
\end{align}
```
Un desarrollo elaborado se puede observar dentro de las <a href={`${import.meta.env.BASE_URL}/proofs/deutsch-jozsa#desarrollo-elaborado-de-la-versión-completa`}>pruebas matemáticas</a>. Finalmente si medimos la amplitud del estado donde $\ket{z}$ es $\ket{0}^{\otimes{n}}$:
```math
\begin{gather}
\left|\frac{1}{2^n}\sum_{x\in\{0,1\}^n} (-1)^{f(x)+x\cdot 00\ldots 0}\right|^2=\left|\frac{1}{2^n}\sum_{x\in\{0,1\}^n} (-1)^{f(x)}\right|^2\\
\end{gather}
```
Estas amplitudes tienen diferentes resultados según si $f$ es constante o balanceada, por un lado si es ***constante***:
```math
\begin{align}
\left|\frac{1}{2^n}\sum_{x\in\{0,1\}^n} \pm 1 \right|^2&=|\pm(\frac{1}{2^n}2^n)|^2 \\
&=|\pm 1|^2\\
&=1\\
\end{align}
```
...
```math
\begin{align}
\left|\frac{1}{2^n}\left(\overbrace{(-1)^0}^{k\text{ veces}}+\overbrace{(-1)^1}^{k\text{ veces}}\right)\right|^2&=\left|\frac{1}{2^n}(1k-1k)\right| & \text{siendo }k=2^{n-1}\\
...
- <a href="https://eprint.iacr.org/2018/249.pdf" target="_blank">Analysis of Deutsch-Jozsa Quantum Algorithm (Zhengjun Cao, Lihua Liu) [Inglés]</a>
- <a href="https://www.cl.cam.ac.uk/teaching/1920/QuantComp/Quantum_Computing_Lecture_7.pdf" target="_blank">Quantum Computing (CST Part II) Lecture 7: Deutsch-Jozsa algorithm (Cambridge) [Inglés]</a>
...
- <a href='https://youtu.be/2wticzHE1vs?si=0kdqs3NTdlArLUPW&t=1328' target='_blank'>
    "Quantum Query Algorithms | Understanding Quantum Information & Computation - Lesson 05" (Qiskit)
    [Inglés]
  </a>
