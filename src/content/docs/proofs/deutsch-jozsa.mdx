...
\end{align}
```
En el último paso eliminamos $\ket{-}$ para simplificar los cálculos ya que no se utiliza en la medición y no interfiere con el resto del algoritmo. Por último detallaremos el estado $\psi_3$:
```math
\begin{align}
