...
```math
H^{\otimes n}\ket{0}^{\otimes n}=\frac{1}{\sqrt{2^n}}\sum_{x\in\{0,1\}^2}\ket{x}
```
Finalmente $\psi_1$ toma el valor:
```math
\psi_1=\frac{1}{\sqrt{2^n}}\sum_{x\in\{0,1\}^2}\ket{x}\ket{-}
```
Luego seguimos con $\psi_2$:
```math
\begin{align}
...
Por último detallaremos el estado $\psi_3$:
```math
\begin{align}
...
=&\frac{1}{2^n}\sum_{x\in\{0,1\}^n} (-1)^{f(x)}\left[\sum_{z\in\{0,1\}^n} (-1)^{x\cdot z} \ket{z}\right]\\
=&\frac{1}{2^n}\sum_{x\in\{0,1\}^n} \sum_{z\in\{0,1\}^n} (-1)^{f(x)}(-1)^{x\cdot z} \ket{z}\\
=&\frac{1}{2^n}\sum_{x\in\{0,1\}^n} \sum_{z\in\{0,1\}^n} (-1)^{f(x)+x\cdot z} \ket{z}\\
=&\frac{1}{2^n}\sum_{z\in\{0,1\}^n} \left(\sum_{x\in\{0,1\}^n} (-1)^{f(x)+x\cdot z}\right) \ket{z}\\
\end{align}
```
